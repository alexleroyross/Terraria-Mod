...
namespace ExampleMod.NPCs
{
	...
	public class CometHead : Hover
	{
		...
		public override void SetDefaults()
		{
			...
			banner = npc.type;
			bannerItem = mod.ItemType("CometHeadBanner");
		}
		...
		public override void CustomBehavior(ref float ai)
		{
			...
			ai += 1f;
            Vector2 newv = new Vector2((float)Math.Sqrt(npc.Center.X + player.Center.X), (float)Math.Sqrt(npc.Center.Y + player.Center.Y));
            double dir = Math.Atan2(newv.Y, newv.X);

            newv.Normalize();
            npc.velocity = newv * 2f;

            // MAY NOT BE RIGHT?
            npc.rotation = (float)dir;
